...
async def get_task(task_id: int, session: AsyncSession = Depends(db_helper.session_dependency)):
    return await crud.get_task(task_id=task_id, session=session)

# @router.patch("/{task_id}")
# async def patch_task(update_task: UpdateTask, task: Task = Depends(get_task_by_id), session: AsyncSession = Depends(db_helper.session)):
